...
if [ -n "$TARGET_REPO" ]; then
    info "Removing project: $target_name"
else
    info "Removing all projects in $PROJECTS_DIR:"
    for repo in "${project_dirs[@]}"; do
        info "- $repo"
    done
fi

info "Running command: $REMOVE_CMD"
if ! $REMOVE_CMD; then
    error "Docker Compose failed to remove containers and images."
    exit 1
fi

success "Drop operation completed successfully."
exit 0
